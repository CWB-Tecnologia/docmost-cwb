// Human-readable labels for the backend AuditEvent constants
// (apps/server/src/common/events/audit-events.ts). Keep in sync with it.

type EventOption = { value: string; label: string };
type EventGroup = { group: string; items: EventOption[] };

export const auditEventLabels: Record<string, string> = {
  "audit.exported": "Exported audit log",
  "audit.integrity_verified": "Verified audit integrity",
  "audit.retention_updated": "Updated audit retention",
  "audit.purged": "Purged expired audit entries",
...
export const eventFilterOptions: EventGroup[] = [
  {
    group: "Audit administration",
    items: [
      { value: "audit.exported", label: "Exported audit log" },
      {
        value: "audit.integrity_verified",
        label: "Verified audit integrity",
      },
      {
        value: "audit.retention_updated",
        label: "Updated audit retention",
      },
      { value: "audit.purged", label: "Purged expired audit entries" },
    ],
  },
  {
    ...
    items: [
      ...
      { value: "user.password_reset", label: "Reset password" },
  ...
  },
  {
    ...
    items: [
      ...
      {
        value: "space.member_role_changed",
        label: "Changed space member role",
      },
